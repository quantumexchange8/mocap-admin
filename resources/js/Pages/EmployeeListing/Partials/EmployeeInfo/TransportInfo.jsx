...
export default function TransportInfo({
    ...
    const { data, setData, errors, post, reset } = useForm({
        id: user_details.transportinfo.id || '',
        approximate_distance: user_details.transportinfo.approximate_distance || '',
        approximate_hours: user_details.transportinfo.approximate_hours || '',
        approximate_minutes: user_details.transportinfo.approximate_minutes || '',
        license_id: user_details.transportinfo.license_id || [],
        own_transport: user_details.transportinfo.own_transport || [],
        work_transportation: user_details.transportinfo.work_transportation || [],
    });

    const closeTransportInfoDialog = () => {
        reset();
        closeTransportInfo();
    }
    ...
    const submit = (e) => {
        ...
        post('/update-transport-info', {
            preserveScroll: true,
            onSuccess: () => {
                setIsLoading(false);
                closeTransportInfoDialog();
                reset();
    ...
    return (
        <>
            <Modal
                ...
                maxWidth='lg'
                title='Transportation and Location Information'
                onClose={closeTransportInfoDialog}
                footer={
                    <div className="flex items-center justify-end gap-4 w-full">
                        <Button variant="outlined" size="sm" onClick={closeTransportInfoDialog}>Cancel</Button>
