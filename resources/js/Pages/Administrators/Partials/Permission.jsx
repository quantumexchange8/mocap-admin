...
export default function Permission({
    ...
    useEffect(() => {
        ...
        if (selectedRowDetail?.permissions?.length > 0) {
            const updatedPermissions = {};
    
            selectedRowDetail.permissions.forEach(permission => {
                // If the permission name exists in the form data, set it to true
    ...
    const updatePermission = (e) => {
        ...
        post('/update-administrator', {
            ...
            onSuccess: () => {
                ...
                toast.success(`Permissions updated successfully for ${selectedRowDetail.name}.`, {
                    title: `Permissions updated successfully for ${selectedRowDetail.name}.`,
                    duration: 3000,
                    variant: 'variant1',
                });
            }
            ...
            <Modal
                ...
                footer={
                    <div className="flex items-center justify-end gap-4 w-full">
                        <Button variant="outlined" size="sm" onClick={closeEditAdminModal}>Cancel</Button>
                        <Button size="sm" onClick={updatePermission}>Save Changes</Button>
                    </div>
                }
            >
                {
                        ...
                        <div className="py-3 px-6 flex flex-col gap-8">
                            <div className="flex items-center gap-3">
                                <div className="max-w-12 min-w-12 w-full h-12 rounded-full">
                                    <img src={selectedRowDetail.profile_image ? selectedRowDetail.profile_image : null} alt="profile_image" className="rounded-full w-12 h-12" />
                                </div>
                                <div className="flex flex-col gap-1">
                                    <div className="text-gray-950 font-semibold text-base">{selectedRowDetail.name}</div>
                                    <div className="text-sm text-gray-500">CTID: {selectedRowDetail.employee_id}</div>
                                </div>
                            </div>
